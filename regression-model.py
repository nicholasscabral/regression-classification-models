...
for r in range(R):
    ...
    XTX = X_treino.T @ X_treino
    ridge_model = np.linalg.inv(XTX + best_alpha(R, X, Y) * np.identity(XTX.shape[0])) @ X_treino.T @ y_treino

    #TESTE DOS MODELOS

    #COLUNA DE '1' NA AMOSTRA DE TESTE PARA COMPARAR E GERAR A PRECISÃO, NO CASO
    #A MÉDIA DOS ERROS QUADRÁTICOS
...
print(MSE_MQO_Tradicional, MSE_Media_Observavel, MSE_MQO_Regularizado)
